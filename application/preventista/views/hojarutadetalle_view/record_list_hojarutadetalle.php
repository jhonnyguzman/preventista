...
$(document).ready(function(){ setPagination('<?=base_url()?>hojarutadetalle_controller/search_c','result-list'); });
</script>
<div id="result-list">
...
<table id="result-set">
	...
	<?php foreach($hojarutadetalle as $f):?>
		<tr>
			...
			<?php else: ?>
				<td><a href="#" onClick="loadPage('<?=base_url()?>index.php/hojarutadetalle_controller/edit_c/<?=$f->hojarutadetalle_id?>','right-content')" id="icon-edit">Modificar</a></td>
			<?php endif;?>
			<?php if(!$flag['d']):?>
